...
func (b *Boomer) runWorker(n int) {
	...
	currentQps := b.WaveForm[0]
	tickDuration := time.Duration(1e9/(currentQps)) * time.Nanosecond
	if b.Qps > 0 {
		//throttle = time.Tick(tickDuration)
		throttleTicker = time.NewTicker(tickDuration)
	}
	...
	for i := 0; i < n; i++ {
		if b.Qps > 0 {
			...
			if(timeElapsed >= b.SamplePeriod) { //move onto the next sample waveform value and reset the ticker to the adjusted delay
				timeElapsed = time.Duration(0)
				sampleIndex = int(math.Mod(float64(sampleIndex + 1), float64(numSamples)))
				currentQps = b.WaveForm[sampleIndex]
				tickDuration = time.Duration(1e6/(currentQps)) * time.Microsecond
				throttleTicker = time.NewTicker(tickDuration)
			}
